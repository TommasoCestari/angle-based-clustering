...
size_t n_of_border_points(const point* points, size_t n_points, float percentile){
    ...
    /*Very easy calculation that is the same for every rank 
      so we only do it once with rank 0 and then broadcast it*/
    if (world_rank == 0){
    ...
    MPI_Bcast(&n_border_points, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD); //MPI_UINT64_T is the same as size_t
